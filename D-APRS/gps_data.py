...
def process_sms(_rf_src, sms):
    ...
    # Write blank entry to cause APRS receive to look for packets for this station.
        ...
        try:
            ...
            dashboard_loc_write(str(get_alias(int_id(_rf_src), subscriber_ids)) + '-' + ssid, aprs_lat, aprs_lon, time.time(), comment, icon_table, icon_icon)
                    ...
                    if btf == 0:
                        ...
                        if '$GPRMC' in final_packet or '$GNRMC' in final_packet:
                            ...
                            # Float values of lat and lon. Anything that is not a number will cause it to fail.
                                ...
                                float(loc.lon)
                                aprs_send(aprs_loc_packet)
                                dashboard_loc_write(str(get_alias(int_id(_rf_src), subscriber_ids)) + '-' + ssid, str(loc.lat[0:7]) + str(loc.lat_dir), str(loc.lon[0:8]) + str(loc.lon_dir), time.time(), comment, icon_table, icon_icon)
                            except Exception as error_exception:
